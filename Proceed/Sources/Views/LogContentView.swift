...
struct TextSelection: Equatable {
    ...
    var normalized: TextSelection {
        if startLine > endLine || (startLine == endLine && startChar > endChar) {
            return TextSelection(startLine: endLine, startChar: endChar, endLine: startLine, endChar: startChar)
        }
        return self
    }
...
final class LogContentView: NSView {
    ...
    var timestampFont: NSFont {
        ...
        return NSFont.monospacedSystemFont(ofSize: min(font.pointSize, maxTimestampFontSize), weight: .regular)
    }
    ...
    func recalculateTotalHeight() {
        ...
        if abs(frame.size.height - newFrame.size.height) > 1 || abs(frame.size.width - newFrame.size.width) > 1 {
            frame = newFrame
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for lineIndex in startLine..<lines.count {
            ...
            drawGutter(timestamp: showTimestamp ? timestamp : nil,
            ...
            // Draw content
            attrString.draw(with: lineBounds, options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    private func drawGutter(timestamp: String?, isLineSelected: Bool, at y: CGFloat, height: CGFloat, in context: CGContext) {
    ...
    private func drawSelectionBackground(for lineIndex: Int, bounds lineBounds: CGRect, in context: CGContext) {
        ...
        if selectedLineIndices.contains(lineIndex) {
            ...
            let rect = CGRect(x: gutterWidth, y: lineBounds.minY,
    ...
    private func lineIndexAndBounds(at point: NSPoint) -> (lineIndex: Int, yStart: CGFloat, height: CGFloat)? {
    ...
    private func handleGutterMouseDragged(at point: NSPoint) {
        guard let anchor = selectionAnchorLine,
              let lineIdx = lineIndex(at: point) else { return }
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        if let word = wordUnderPoint(point) {
            menu.addItem(NSMenuItem.separator())
            let lookUpItem = NSMenuItem(title: "Look Up \"\(word)\"", action: #selector(lookUpWord(_:)), keyEquivalent: "")
...
struct LogContentViewRepresentable: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        contentView.onHeightChanged = { [weak coordinator, weak contentView] in
            guard let coordinator = coordinator,
                  ...
                  coordinator.isTailingBinding?.wrappedValue == true else { return }
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        } else if newCount != oldCount || (newCount > 0 && lines.first?.id != contentView.lines.first?.id) {
