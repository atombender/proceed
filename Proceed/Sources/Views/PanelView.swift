...
struct PanelView: View {
    ...
    private var contentArea: some View {
        ...
        .background {
            Button("") { settingsManager.fontSize += 1 }.keyboardShortcut("+", modifiers: .command).opacity(0)
            Button("") { settingsManager.fontSize = max(6, settingsManager.fontSize - 1) }.keyboardShortcut("-", modifiers: .command).opacity(0)
        }
    }
}
