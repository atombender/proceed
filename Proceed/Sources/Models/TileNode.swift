...
enum TileNode: Identifiable, Equatable {
    ...
    case split(id: UUID, direction: SplitDirection, first: Box<TileNode>, second: Box<TileNode>, ratio: CGFloat)
    ...
    static func == (lhs: TileNode, rhs: TileNode) -> Bool {
        ...
        case (.split(let id1, let dir1, let first1, let second1, let ratio1),
              .split(let id2, let dir2, let first2, let second2, let ratio2)):
            return id1 == id2 && dir1 == dir2 && first1 == first2 && second1 == second2 && ratio1 == ratio2
        default:
            return false
        }
    }
    ...
    func insertingAsSplit(
        ...
        case .split(let id, let direction, let first, let second, let ratio):
            return .split(
                id: id,
                direction: direction,
                first: Box(first.value.insertingAsSplit(newPanelId: newPanelId, relativeTo: targetPanelId, position: position)),
                second: Box(second.value.insertingAsSplit(newPanelId: newPanelId, relativeTo: targetPanelId, position: position)),
                ratio: ratio
            )
        }
    }
    ...
    func redundantDropPositions(dragging draggedPanelId: UUID, onto targetPanelId: UUID) -> Set<DropPosition> {
        switch self {
        case .leaf:
            return []
        case .split(_, let direction, let first, let second, _):
            ...
            let fromFirst = first.value.redundantDropPositions(dragging: draggedPanelId, onto: targetPanelId)
