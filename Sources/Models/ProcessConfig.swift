...
class RunningProcess: ObservableObject, Identifiable {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    self.panel?.appendEvent(.failed, message: "Failed to start: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    private func startOutputStreaming(backend: TmuxBackend, handle: ProcessHandle, fromBeginning: Bool) {
        // Use batched callback for better performance with high-volume output
        backend.startOutputBatchCallback(for: handle, fromBeginning: fromBeginning) { [weak self] lines in
