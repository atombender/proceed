...
class PersistenceManager {
    ...
    private func createTables() {
        ...
        if sqlite3_prepare_v2(db, checkSql, -1, &stmt, nil) == SQLITE_OK {
            if sqlite3_step(stmt) == SQLITE_ROW {
                ...
                if count == 0 {
                    sqlite3_exec(db, "ALTER TABLE log_lines ADD COLUMN kind INTEGER NOT NULL DEFAULT 0", nil, nil, nil)
                }
            }
        }
    ...
    func appendToLog(panelId: UUID, line: String, timestamp: Date = Date(), kind: LogEntryKind = .output) {
        ...
        writeQueue.async { [weak self] in
            ...
            if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
                sqlite3_bind_text(stmt, 1, panelIdStr, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
    ...
    func appendToLog(panelId: UUID, lines: [String], timestamps: [Date]? = nil, kind: LogEntryKind = .output) {
        guard !lines.isEmpty else { return }

        let panelIdStr = panelId.uuidString
        let kindVal = Int32(kind.rawValue)
        // Capture timestamps as intervals to avoid Date reference issues
        let timestampVals: [TimeInterval] = timestamps?.map { $0.timeIntervalSince1970 }
    ...
    func readLog(for panelId: UUID, limit: Int = 10000) -> [(text: String, timestamp: Date, kind: LogEntryKind)] {
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            sqlite3_bind_text(stmt, 1, panelId.uuidString, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
    ...
    func recordRun(name: String, command: String, workingDirectory: String, shell: String) {
        let sql = "INSERT INTO run_history (name, command, working_directory, shell, started_at) VALUES (?, ?, ?, ?, ?)"
        var stmt: OpaquePointer?

        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            sqlite3_bind_text(stmt, 1, name, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
            sqlite3_bind_text(stmt, 2, command, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
            sqlite3_bind_text(stmt, 3, workingDirectory, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self))
    ...
    func getRecentRuns(limit: Int = 10) -> [RunHistoryEntry] {
        ...
        if sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK {
            ...
            while sqlite3_step(stmt) == SQLITE_ROW {
                ...
                results.append(RunHistoryEntry(
                    id: id,
                    name: name,
                    command: command,
...
struct GlobalSettings: Codable {
    ...
    init(theme: AppTheme = .auto, fontSize: CGFloat = 12, maxLineHistory: Int = 10000, autoDirenv: Bool = false) {
        self.theme = theme
...
struct AppState: Codable {
    ...
    init(panels: [PanelState] = [], layout: LayoutNode? = nil, lastWorkingDirectory: String = "", lastCommand: String = "") {
        self.panels = panels
        self.layout = layout
...
indirect enum LayoutNode: Codable {
    ...
    case split(id: UUID, direction: LayoutDirection, first: LayoutNode, second: LayoutNode, ratio: CGFloat)
}
