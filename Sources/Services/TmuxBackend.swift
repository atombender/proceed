...
final class TmuxBackend: ProcessBackend {
    ...
    func kill(handle: ProcessHandle) async throws {
        ...
        if result.exitCode != 0 && !result.stderr.contains("no server running") && !result.stderr.contains("session not found") {
    ...
    func isRunning(handle: ProcessHandle) async -> Bool {
        ...
        guard let result = try? await runTmux([
    ...
    func startOutputCallback(for handle: ProcessHandle, fromBeginning: Bool, onLine: @escaping (String) -> Void) {
        let reader = PipeReader(path: handle.pipePath, fromBeginning: fromBeginning, onLine: onLine, onLines: nil)
    ...
    func startOutputBatchCallback(for handle: ProcessHandle, fromBeginning: Bool, onLines: @escaping ([String]) -> Void) {
        let reader = PipeReader(path: handle.pipePath, fromBeginning: fromBeginning, onLine: nil, onLines: onLines)
    ...
    private func startPipeReader(
        ...
        let reader = PipeReader(path: pipePath, fromBeginning: fromBeginning, onLine: { line in
...
extension ProcessConfig: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.init(id: id, name: name, command: command, workingDirectory: workingDirectory, shell: shell)
    }
...
private class PipeReader {
    ...
    private func readNewContent() {
        ...
        while let lfIndex = dataBuffer.firstIndex(of: PipeReader.LF) {
            var lineEnd = lfIndex
            // Check for \r\n (strip the \r)
            if lineEnd > dataBuffer.startIndex && dataBuffer[dataBuffer.index(before: lineEnd)] == PipeReader.CR {
