...
struct PanelWithDropZones: View {
  ...
  private func dropHighlightRect(for position: DropPosition, in size: CGSize) -> some View {
    ...
    case .right:
      rect = CGRect(
        x: size.width * (1 - edgeThreshold), y: 0, width: size.width * edgeThreshold,
        height: size.height)
    case .top:
      rect = CGRect(x: 0, y: 0, width: size.width, height: size.height * edgeThreshold)
    case .bottom:
      rect = CGRect(
        x: 0, y: size.height * (1 - edgeThreshold), width: size.width,
        height: size.height * edgeThreshold)
    case .center:
      rect = CGRect(
        x: size.width * edgeThreshold, y: size.height * edgeThreshold,
